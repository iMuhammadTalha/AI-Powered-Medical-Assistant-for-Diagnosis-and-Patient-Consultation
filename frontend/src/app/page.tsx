      ...
      <div className="bg-white shadow-lg rounded-lg p-6 w-full max-w-2xl">
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="col-span-2 flex flex-col">
            <label className="text-sm font-medium text-black">Query <span className="text-red-500">*</span></label>
            <textarea
              name="query"
          ...
          {Object.keys(formData).map((key) =>
            key !== "query" ? (
              <div key={key} className="flex flex-col">
                <label className="text-sm font-medium text-black capitalize">{key.replace(/([A-Z])/g, " $1")}</label>
                <input
                  type="text"
                  name={key}
        ...
        {audioUrl && (
          <div className="mt-4">
            <h2 className="text-lg font-medium text-black">Audio Response</h2>
            <audio controls>
